...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        Method *methodList = class_copyMethodList([NSURLSession class], &methodCount);
        Method origin = nil;
        Method custom = nil;
        //查找我们需要替换的两个函数
        for (NSInteger i = 0; i < methodCount; i++) {
            ...
            if ([@"initWithConfiguration:delegate:delegateQueue:" isEqualToString:methodName]) {
                origin = method;
            }
            if ([@"hg_initWithConfiguration:delegate:delegateQueue:" isEqualToString:methodName]) {
                custom = method;
            }
            
            //都找到了，退出循环
            if (nil != origin && nil != custom) {
                break;
            }
        }
        ...
        //替换
        if (nil != origin && nil != custom) {
            //HGLogInfo(kLogTag, @"NSURLSession load found method,exchange");
            method_exchangeImplementations(origin, custom);
        }
    });
}

- (NSURLSession *)hg_initWithConfiguration:(NSURLSessionConfiguration *)configuration delegate:(id<NSURLSessionDelegate>)delegate delegateQueue:(NSOperationQueue *)queue {
    //HGLogInfo(kLogTag,@"hg_initWithConfiguration");
    //注册我们的类
    configuration.protocolClasses = @[ [CFHttpMessageURLProtocol class] ];
    return [self hg_initWithConfiguration:configuration delegate:delegate delegateQueue:queue];
}
@end
